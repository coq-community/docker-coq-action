...
exec 2>&1 ; endGroup () {  {  init_opts=\"\$-\"; set +x ; } 2> /dev/null; if [ -n \"\$_startTime\" ]; then _endTime=\$(date -u +%s); echo \"::endgroup::\"; printf \"↳ \"; date -u -d \"@\$((_endTime - _startTime))\" '+%-Hh %-Mm %-Ss'; echo; unset _startTime; else echo 'Error: missing startGroup command.'; case \"\$init_opts\" in  *x*) set -x ;; esac; return 1; fi; case \"\$init_opts\" in  *x*) set -x ;; esac; } ; startGroup () {  {  init_opts=\"\$-\"; set +x ; } 2> /dev/null; if [ -n \"\$_startTime\" ]; then endGroup; fi; if [ \$# -ge 1 ]; then _groupTitle=\"\$*\"; else _groupTitle=\"Unnamed group\"; fi; echo; echo \"::group::\$_groupTitle\"; unset _groupTitle; _startTime=\$(date -u +%s); case \"\$init_opts\" in  *x*) set -x ;; esac; } # generated from helper.sh
